...
void DaikinClimate::transmit_state() {
  ...
  for (uint8_t i = 0; i < DAIKIN_LEADER_BITS; i++) {
    data->mark(DAIKIN_BIT_MARK);
    data->space(DAIKIN_ZERO_SPACE);
  }
  data->mark(DAIKIN_BIT_MARK);
  data->space(DAIKIN_LEADER_SPACE);
...
bool DaikinClimate::on_receive(remote_base::RemoteReceiveData data) {
  ...
  for (uint8_t pos = 0; pos < DAIKIN_STATE_FRAME_SIZE; pos++) {
    ...
    for (int8_t bit = 0; bit < 8; bit++) {
      if (!data.is_valid(1))
        return false;
      const int32_t mark = data.peek();
      const int32_t space = -data.peek(1);
      if (mark <= 0 || mark >= int32_t(DAIKIN_BIT_THRESHOLD) || space <= 0 || space >= int32_t(DAIKIN_MESSAGE_SPACE))
        return false;
      if (space > int32_t(DAIKIN_BIT_THRESHOLD))
        byte |= 1 << bit;
      data.advance(2);
    }
